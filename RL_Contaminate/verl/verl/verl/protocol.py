...
def union_numpy_dict(tensor_dict1: dict[np.ndarray], tensor_dict2: dict[np.ndarray]) -> dict[np.ndarray]:
    for key, val in tensor_dict2.items():
        if key in tensor_dict1:
            ...
            # print(np.all(tensor_dict2[key] == tensor_dict1[key]))

            # try:
            #     # 打印关键信息
            #     print(f"Processing key: '{key}'")
            #     val1 = tensor_dict1[key]
            #     val2 = tensor_dict2[key]
            #     print(f"  - val1 type: {type(val1)}, dtype: {getattr(val1, 'dtype', 'N/A')}, shape: {getattr(val1, 'shape', 'N/A')}")
            #     print(f"  - val2 type: {type(val2)}, dtype: {getattr(val2, 'dtype', 'N/A')}, shape: {getattr(val2, 'shape', 'N/A')}")

            #     # 尝试进行比较
            #     comparison = (val1 == val2)
            #     print(f"  - Comparison result type: {type(comparison)}, dtype: {getattr(comparison, 'dtype', 'N/A')}")
            ...
            #     # 执行原始的断言
            #     assert are_equal, f"Values for key '{key}' are not equal."
            ...
            are_equal = False
            # 检查是否是 object 类型的 ndarray，这通常是嵌套数组的标志
            if isinstance(val1, np.ndarray) and val1.dtype == 'object':
                # 如果是嵌套数组，我们需要更复杂的比较逻辑
                try:
                    # 检查两个顶层数组的长度是否一致
                    if len(val1) != len(val2):
                        ...
                        # 逐个比较内部的数组
                        # all() 会对一个生成器表达式进行求值
                        are_equal = all(np.array_equal(sub_arr1, sub_arr2) 
                                        for sub_arr1, sub_arr2 in zip(val1, val2))
                except Exception:
                    # 如果内部元素不是数组或比较失败，则认为不相等
                    are_equal = False
            else:
                # 对于常规的 NumPy 数组，使用 np.array_equal
                # np.array_equal 比 np.all(arr1 == arr2) 更安全，能处理形状不匹配等情况
                are_equal = np.array_equal(val1, val2)

            assert are_equal, f"Values for key '{key}' do not match. \nval1:\n{val1}\n\nval2:\n{val2}"
